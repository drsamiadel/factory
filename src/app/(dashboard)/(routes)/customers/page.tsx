...
import { CREATE, DELETE, UPDATE } from '../../../../../actions/customer';
import { Customer, Image as ImageType, Supplier, User } from '@prisma/client';
...
interface CustomerWithUser extends Partial<Customer> {
    user: Partial<User>;
}
...
export default function CustomizedTables() {
    ...
    const [rows, setRows] = React.useState<CustomerWithUser[] | []>([]);
    const [open, setOpen] = React.useState(false);
    const [selectedId, setSelectedId] = React.useState<string>("");
    const [loading, setLoading] = React.useState<boolean>(true);
    const [order, setOrder] = React.useState<Order>('desc');
    const [orderBy, setOrderBy] = React.useState<keyof CustomerWithUser>('createdAt');

    const handleRequestSort = (
        event: React.MouseEvent<unknown>,
        property: keyof CustomerWithUser,
    ) => {
    ...
    React.useEffect(() => {
        const fetchInputs = async () => {
            ...
            const response = await fetch(`/api/data/customer?filterByName=${debouncedSearch}&page=${page + 1}&limit=${rowsPerPage}&sorting=${orderBy}&order=${order}`);
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                                ...
                                >
                                    {headCell.id === 'name' || headCell.id === 'code' || headCell.id === 'createdAt' ? (
                                        <TableSortLabel
                                            ...
                                            onClick={(event) => handleRequestSort(event, headCell.id as keyof CustomerWithUser)}
                                            disabled={loading}
                                        >
                                            {headCell.label}
                                        </TableSortLabel>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <StyledTableRow>
                                <StyledTableCell colSpan={9} align="center">No data found</StyledTableCell>
                            </StyledTableRow>
                            :
                            rows.map((row) => (
