...
export default function CustomizedTables() {
    ...
    const [rows, setRows] = React.useState<SupplierWithUser[] | []>([]);
    const [open, setOpen] = React.useState(false);
    const [selectedId, setSelectedId] = React.useState<string>("");
    const [loading, setLoading] = React.useState<boolean>(true);
    const [order, setOrder] = React.useState<Order>('desc');
    const [orderBy, setOrderBy] = React.useState<keyof SupplierWithUser>('createdAt');

    const handleRequestSort = (
        event: React.MouseEvent<unknown>,
        property: keyof SupplierWithUser,
    ) => {
    ...
    React.useEffect(() => {
        const fetchInputs = async () => {
            ...
            const response = await fetch(`/api/data/supplier?filterByName=${debouncedSearch}&page=${page + 1}&limit=${rowsPerPage}&sorting=${orderBy}&order=${order}`);
    ...
    const handleDelete = async (id: string) => {
        await DELETE(id).then((res) => {
            ...
            setRows(updatedRows as SupplierWithUser[]);
        }).catch((err) => {
            console.log(err);
        });
    };
    ...
    const handleCreate = async (data: any) => {
        try {
            await CREATE(data).then((res) => {
                setRows([res as SupplierWithUser, ...rows]);
            })
        } catch (error) {
            throw error;
        }
                                ...
                                >
                                    {headCell.id === 'name' || headCell.id === 'code' || headCell.id === 'createdAt' ? (
                                        <StyledTableSortLabel
                                            ...
                                            onClick={(event) => handleRequestSort(event, headCell.id as keyof SupplierWithUser)}
                                            disabled={loading}
                                        >
                                            {headCell.label}
                                        </StyledTableSortLabel>
